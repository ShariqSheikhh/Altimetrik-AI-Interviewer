  ...
  useEffect(() => {
    ...
    const getUrl = async () => {
      try {
        ...
        if (data.signedUrl) setPresignedUrl(data.signedUrl);
        ...
        <aside className="space-y-8 min-w-0">
          ...
          <div className="bg-white border border-slate-200 rounded-[2.5rem] overflow-hidden shadow-sm">
            <div className="p-5 bg-slate-50/50 border-b border-slate-100 flex items-center justify-between">
              <h3 className="font-bold text-slate-900 flex items-center gap-2 uppercase tracking-widest text-[11px]">
                <Video size={14} className="text-blue-500" /> Recording
              </h3>
              {result.video_url && !videoError && presignedUrl && (
                <div className="flex items-center gap-1 px-2 py-1 bg-emerald-50 text-emerald-600 rounded-lg text-[10px] font-black tracking-widest">
            ...
            <div className="aspect-video bg-slate-900">
              {!result.video_url ? (
                <div className="h-full flex flex-col items-center justify-center p-6 text-center">
                  ...
                  <p className="text-slate-500 text-xs font-bold">No recording captured</p>
                </div>
              ) : !presignedUrl ? (
                ...
                <video src={presignedUrl} controls className="w-full h-full" preload="metadata" onError={() => setVideoError(true)} />
              )}
            </div>
          </div>
